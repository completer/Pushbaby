...
(2) Secure by default
Pushbaby traffic is encrypted, and runs your deployment script on the destination server, not from the build server.
...
1. On the build server, the Pushbaby.Client pushes your build output to the destination server(s).
2. On the destination server, Pushbaby.Service executes your deployment script.
3. The script's standard output is streamed back to your build server through Pushbaby.Client.
